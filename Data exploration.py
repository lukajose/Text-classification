...
df2.plot.barh(
    x="topic",
    y="count",
    title="Number of documents per topic (train set)",
    color=[
        "black",
        "red",
        "green",
...
for topic in topics:
    ...
    sorted_word_freq = [
        (k, v)
        for k, v in sorted(word_freq.items(), reverse=True, key=lambda item: item[1])
    ]
...
lr = LogisticRegression(
    multi_class="multinomial", solver="lbfgs", max_iter=1000, random_state=0
)
mnb = MultinomialNB()
